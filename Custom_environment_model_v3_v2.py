...
Environment model : V3

v3: one agent per sortie
...
class UAV_UGV_Env(gym.Env):
    ...
    def step(self, actions, step):
        ...
        # Check if any UAV in each batch has depleted fuel
        ...
        self.mission_status[batch_indices, unique_action_indices] = 1                              # updates age period of visited mission points as 0
        ...
        if uav_agent_mask.any():  # If any instance in the batch is UAV
            ...
            if not_all_landed_mask.any():
                ...
                if recharge_mask.any():
                    filtered_mission_times = self.mission_time_elapsed_local_uav.clone()
                    filtered_mission_times[self.uavs_landing_complete_flag.bool()] = float('inf')  # Mask out landed UAVs by setting their time to infinity
                    min_time_indices = torch.argmin(filtered_mission_times, dim=1)  # Shape: [B]
        ...
        step_condition = torch.tensor([step > 50], dtype=torch.bool, device = device)
        ...
        if self.mission_complete_flag[0] == 1:
        ...
        if dones.all():
           ...
           print(f"Number of unfinished batch indices: {unfinished_batch_indices.numel()}")
    ...
    def reset(self, input):
         ...
         No_of_UAVs = opts.No_of_UAVs
         No_of_UGVs = opts.No_of_UGVs
