# -*- coding: utf-8 -*-
"""
Created on Sun Nov 12 11:10:54 2023
...
'''
Evaluation for greedy decoding 
'''
...
def eval_dataset(dataset_path, width, softmax_temp, opts):
    ...
    if opts.DRL_model == 'MF':
        full_path = os.path.join(folder,'actions_RL_greedy_{}_{}_MF.csv'.format(opts.No_of_UAVs, opts.No_of_UGVs))
    else :
        full_path = os.path.join(folder,'actions_RL_greedy_{}_{}.csv'.format(opts.No_of_UAVs, opts.No_of_UGVs))
...
def _eval_dataset(model, dataset, width, softmax_temp, opts, device):
    ...
    folder = r'./Tours_output'
    full_path = os.path.join(folder,'scenarios_copy.csv')
    ...
    for batch_id, batch in enumerate (tqdm(dataloader, disable=opts.no_progress_bar)) :
        ...
        df.to_csv(full_path, mode = 'a', index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ugv_graph_size', type=int, default=15, help="The size of the uav mission points graph")
    ...
    parser.add_argument('--val_size', type=int, default=256*1,
