...
import copy
from train_modified import rollout, get_inner_model
import time
    ...
    def _update_model(self, model, epoch, dataset=None):
        ...
        if dataset is not None:
            if len(dataset) != self.opts.val_size:
                ...
                dataset = None
            elif (dataset[0] if self.problem.NAME == 'tsp' else dataset[0]['uav loc']).size(0) != self.opts.uav_graph_size:
                print("Warning: not using saved baseline dataset since graph_size does not match")
                dataset = None

        if dataset is None:
            st = time.time()
            self.dataset = self.problem.make_dataset( uav_graph_size=self.opts.uav_graph_size, ugv_graph_size=self.opts.ugv_graph_size,  num_samples=self.opts.val_size)
            print('Dataset generation time ----->', (time.time()-st))
        else:
